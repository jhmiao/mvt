...
def gurobi_solve():
    ...
    # Each event happens during its feasible time window
    for i in range(m):
        model.addConstr(sum(t[i][d] for d in range(block)) >= 30)
        for d in range(block):
...
# improve by setting x_ij == 0 if i == j
def gurobi_solve1():
    iterations = []
    objective_values = []
...
def gurobi_solve2():
    ...
    for i in range(m):
        for d in range(block):
            # set delta[i][d][w] = 0 if the event is not scheduled
            if time_window[i][d][1] == 0:
                for w in range(nr):
                    model.addConstr(delta[i][d][w] == 0)
            else:
                for w in range(nr):
