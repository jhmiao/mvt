...
def gurobi_solve():
    ...
    # at most 2 events per day for each person
    model.addConstrs(gp.quicksum(x[i,j,d,w] for i in range(m) for j in range(m)) <= 1 for d in range(block) for w in range(n))
    ...
    model.setParam('LogFile', 'gurobi_output_2.txt')
    ...
    if model.SolCount > 0:
        ...
        file_path1 = f'/Users/jinghongmiao/Code/mvt-code/result-250414/2_EVENT_{nr}_{nl}_{m}_seed{seed_number}.txt'
        file_path2 = f'/Users/jinghongmiao/Code/mvt-code/result-250414/2_NURSE_{nr}_{nl}_{m}_seed{seed_number}.txt'
