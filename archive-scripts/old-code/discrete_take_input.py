...
def gurobi_solve():
    ...
    # f_j: vector of length T * block, f_j[t] = 1 if event j is scheduled during at time t
    ...
    for j in range(m):
        model.addConstr(sum(g[j][t] for t in range(T*block)) == 1)

    # Each event is scheduled exactly once during its feasible time window
    for j in range(m):
        for t in range(T*block):
            model.addConstr(f[j][t] == sum(g[j][s] for s in range(np.maximum(t - C_dur[j]+1, 0), t+1))) 

    # nurse goes to an event only when the event is scheduled
    for j in range (m):
        for w in range(nr+nl):
            for t in range(T*block):
                model.addConstr(x[j][t][w] <= f[j][t])
                model.addConstr(y[j][t][w] <= g[j][t])
    ...
    for j in range(m):
        for t in range(T*block):
            if time_window[j][t] == 0:
                model.addConstr(g[j][t] == 0)

    # Minimum working hours (20 hours per week)
    for w in range(nr+nl):
        model.addConstr(sum(x[j][t][w] for j in range(m) for t in range(T*block)) >= 10)
        ...
        with open(f"input_{nr}_{nl}_{m}_seed{seed_number}.txt", "w") as file:
            ...
            for j in range(m):
                for d in range(block):
                    ...
                    sid = sum(g[j][t].x for t in range(d*T, (d+1)*T))
                    tid = sum((g[j][t].x * t_vector[t]) for t in range(d*T, (d+1)*T))
                    if sid > 0:
                        file.write(f"s_{j}_{d}, {sid}\n")
                    if tid > 0:
                        file.write(f"t_{j}_{d}, {tid}\n")
            
            # write alpha_i_d_w where alpha_i_d_w = 1 if nurse w is picking up equipment for event i on day d
            # write beta_i_d_w where beta_i_d_w = 1 if nurse w is dropping off equipment for event i on day d
            for j in range(m):
                for d in range(block):
                    sid = sum(g[j][t].x for t in range(d*T, (d+1)*T))
                    if sid > 0:
                        for w in range(nr+nl):
                            if alpha[j][w].x == 1:
                                file.write(f"alpha_{j}_{d}_{w}, {alpha[j][w].x}\n")
                            if beta[j][w].x == 1:
                                file.write(f"beta_{j}_{d}_{w}, {beta[j][w].x}\n")
            ...
            for w in range(nr+nl):
                ...
                for d in range(block):
                    ...
                    if event_1 is not None:
                        ...
                        if xdepot_event_1 == 1:
                            file.write(f"x_-1_{event_1}_{d}_{w}, {xdepot_event_1}\n")

                        if xhome_event_1 == 1:
                            file.write(f"x_-2_{event_1}_{d}_{w}, {xhome_event_1}\n")

                        if xevent_home_1 == 1:
                            file.write(f"x_{event_1}_-2_{d}_{w}, {xevent_home_1}\n")

                        if xevent_depot_1 == 1:
                            file.write(f"x_{event_1}_-1_{d}_{w}, {xevent_depot_1}\n")
                        ...
                        if event_2 is not None:
                            ...
                            if xevent_depot_2 == 1:
                                file.write(f"x_{event_2}_-1_{d}_{w}, {xevent_depot_2}\n")

                            if xevent_home_2 == 1:
                                file.write(f"x_{event_2}_-2_{d}_{w}, {xevent_home_2}\n")
