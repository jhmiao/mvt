...
def gurobi_solve():
    ...
    model.addConstrs(
        (gp.quicksum(x[j, t, w] for j in range(m) for t in range(T * block)) >= 10 for w in range(n)),
        name="minimum_hours"
    )
    ...
    if model.SolCount > 0:
        ...
        with open(f"discrete_{nr}_{nl}_{m}_seed{seed_number}_2event.txt", "w") as file:
        ...
        with open(f"input_{nr}_{nl}_{m}_seed{seed_number}_2event.txt", "w") as file:
