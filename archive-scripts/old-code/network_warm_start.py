...
def gurobi_solve():
    ...
    for w in range(n):
        model.addConstr(sum(C_dur[j] * 
                    sum(
                        ...
                        ) for j in range(m)) >= 10 * 30, name=f"R{w}_min_hours") 

    # Minimum 2 events per day for each nurse
    for d in range(block):
        for w in range(n):
            model.addConstr(sum(x[i][j][d][w] for i in range(m) for j in range(m)) >= 1, name=f"R{d}_{w}_min_2_events")
    ...
    model.Params.Threads = 8  # Use only 4 CPU cores
    ...
    if model.SolCount > 0:
        ...
        with open(f"network_ws_{nr}_{nl}_{m}_seed{seed_number}.txt", "w") as file:
        ...
        with open(f"network_schedule_{nr}_{nl}_{m}_seed{seed_number}.txt", "w") as file:
            file.write(f"Best feasible solution found within {time_limit} seconds:\n")
            # Write the objective value to the file
            file.write(f"\nObjective value: {model.ObjVal}\n")
            for w in range(n):
