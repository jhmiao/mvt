...
# Read the Excel file
file_path = 'input_parameters_real_ampm.xlsx'
...
def gurobi_solve():
    ...
    for i in range(m):
        for j in range(m):
            for d in range(block):
                if (time_window[i][d][0]+ time_window[i][d][1]) * (time_window[j][d][0]+ time_window[j][d][1]) == 0:
                    model.addConstrs(
                        (x[i, j, d, w] == 0 for w in range(n)),
                        name=f"R{i}_{j}_{d}_day_trip_infeasible"
                    )
    
    # 3) set all flow to 0 if time window is infeasible
    for d in range(block):
        for i in range(m):
            if time_window[i][d][0] == 0 and time_window[i][d][1] == 0:
                ...
                model.addConstrs(
                    (x[i, j, d, w] == 0 for j in range(m+2) for w in range(n)),
                    name=f"R{i}_{d}_time_window_infeasible"
                )
                model.addConstrs(
                    (x[j, i, d, w] == 0 for j in range(m+2) for w in range(n)),
                    name=f"R{i}_{d}_time_window_infeasible_2"
                )
    ...
    model.addConstrs(
        (alpha[j, d, w] <= 2 * x[m, m+1, d, w] for j in range(m) for d in range(block) for w in range(n)),
        name="pick_up_leader_home_depot"
    )

    model.addConstrs(
        (beta[j, d, w] <= 2 * x[m+1, m, d, w] for j in range(m) for d in range(block) for w in range(n)),
        name="drop_off_leader_depot_home"
    )

    model.addConstrs(
        (x[m, m+1, d, w] <= gp.quicksum(alpha[j, d, w] for j in range(m)) for d in range(block) for w in range(n)),
        name="pick_up_leader_home_depot_2"
    )

    model.addConstrs(
        (x[m+1, m, d, w] <= gp.quicksum(beta[j, d, w] for j in range(m)) for d in range(block) for w in range(n)),
        name="drop_off_leader_depot_home_2"
    )
            ...
            for d in range(block):
                ...
                for i in range(m):
                    if s[i,d].x == 1:
                        ...
                        for w in range(n):
                            if alpha[i,d,w].x == 1:
                                file.write(f"Pick up by Nurse {w+1} \n")
                            if beta[i,d,w].x == 1:
                                file.write(f"Drop off by Nurse {w+1} \n")
                            if sum(x[i,j,d,w].x for j in range(m+2)) >= 1 and w < nr:
                                file.write(f"Nurse {w+1} (RN)\n")
                            elif sum(x[i,j,d,w].x for j in range(m+2)) >= 1 and w >= nr:
                                file.write(f"Nurse {w+1} (LVN)\n")
        
        with open(f"ampm_nurse_real.txt", "w") as file:
            file.write(f"Best feasible solution found within {time_limit} seconds:\n")
            # Write the objective value to the file
            file.write(f"\nObjective value: {model.ObjVal}\n")
            file.write(f"the gap is {model.MIPGap}\n")
            for w in range(n):
                ...
                for d in range(block):
                    ...
                    for i in range(m+2):
                        for j in range(m+2):
                            if x[i,j,d,w].x == 1:
                                day_traveltime += C_event[i,j] if i < m and j < m else C_depot[i] if i == m+1 else C_home[j]
                    print(f"Total travel time: {day_traveltime} minutes")
