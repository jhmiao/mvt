...
def generate_time_window():
    start = np.random.randint(0, 11) # Start time: 0 to 300 by 30s
    end = np.random.randint(start + 1, 12)  # End time: start + 30 to 330 by 30s
    return [start, end]
...
def cp_solve():
    ...
    t = [[model.NewIntVar(0, 12, f"t_{i}{d}") for d in range(block)] for i in range(m)]
    ...
    # Create a solution printer and solve the model
    solver = cp_model.CpSolver()
    solution_printer = VarArraySolutionPrinter([s, t, x, xhome_event, xevent_home, xhome_depot, xdepot_home, xdepot_event, xevent_depot, delta])
...
def gurobi_solve():
    ...
    # t_id time when event i starts on day d
    t = [[model.addVar(0, 12, vtype=GRB.INTEGER, name=f"t_{i}{d}") for d in range(block)] for i in range(m)]

    # delta_idw = 1 if nurse w (RN) is the team leader for event i on day d, 0 otherwise
    delta = [[[model.addVar(vtype=GRB.BINARY, name=f"delta_{i}{d}{w}") for w in range(nr)] for d in range(block)] for i in range(m)]
    ...
    #         for d in range(block):
    #             for w in range(nr+nl):
    #                 model.addConstr(t[j][d] >= t[i][d] + C_dur[i] + C_event[i][j] - 390 * (1 - x[i][j][d][w]))
    ...
    for i in range(m):
        # exactly one RN is the team leader
        model.addConstr(sum(delta[i][d][w] for w in range(nr) for d in range(block)) == 1)
        for d in range(block):
            for w in range(nr):
                # team leader happens only if the event is scheduled
                # model.addConstr(delta[i][d][w] <= s[i][d])
                # team leader is the RN who goes to the event
                model.addConstr(delta[i][d][w] <= sum(x[i][j][d][w] for j in range(m)) + xevent_home[i][d][w] + xevent_depot[i][d][w])
    ...
    for w in range(nr):
        for d in range(block):
            ...
            # team leader goes from home to depot to their first event if they are the team leader for some event on that day
            for i in range(m):
                model.addConstr(sum(delta[i][d][w] for i in range(m)) <= 10 * xhome_depot[d][w])
    ...
    if model.SolCount > 0:
        ...
        with open("optimization_results.txt", "w") as file:
            file.write("Best feasible solution found within 1 minutes:\n")
            for d in range(block):
                file.write(f"Day {d+1}:\n")
                for i in range(m):
                    if s[i][d].x == 1:
                        file.write(f"Event {i+1} scheduled at {get_time(t[i][d].x)}\n")
