...
def iterative_pick_day_by_lp_relaxation(
    ...
    *,
    ...
    """
    Iteratively fix s[i,d] from LP-relaxation:
    1) Fix every unfixed i with any s[i,d] == 1 (within tolerance).
    2) If none qualify, fix one i with maximum (largest - second_largest) gap.
    Repeat until all events are fixed.
    """
    ...
    while np.any(fixed_day < 0):
        ...
        # Rule 1: fix all rows that already have a 1.
        for i in unfixed:
            row = s_lp[i]
            ones = np.where(np.isclose(row, 1.0, atol=one_tol))[0]
            if len(ones) > 0:
                d = int(ones[0])
                to_fix.append((i, d, "rule1_exact_one"))

        # Rule 2: if none from rule 1, fix one with maximum top1-top2 gap.
        if not to_fix:
            best_i = -1
            best_d = -1
            best_gap = -np.inf
            for i in unfixed:
                ...
                if gap > best_gap:
                    ...
                    best_i = i
                    best_d = d
            if best_i < 0 or best_d < 0:
                raise RuntimeError("Failed to select event/day in rule 2.")
            to_fix.append((best_i, best_d, "rule2_largest_gap"))
