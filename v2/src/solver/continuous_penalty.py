...
def continuous_algorithm_penalty (data: ProblemData, work_limit, seed_number, multiple_tw=None, event_limit=None, pruning=False, min_hour=None, max_hour=None):
    ...
    u_work = model.addVars(n, lb=0.0, vtype=GRB.CONTINUOUS, name="u_work")
    ...
    model.addConstrs(
        (
            ...
            gp.quicksum(
                C_dur[j] *
                gp.quicksum(x[i, j, d, w] for i in range(m+3) for d in range(day))
                for j in range(m)
            ) - max_hour[w]*60
        )
        for w in range(n)
    ...
    penalty_work = lam * gp.quicksum(u_work[w] for w in range(n))

    objective = event_cost + home_cost + depot_event_cost + depot_home_cost

    model.setObjective(objective + penalty_work, GRB.MINIMIZE)
...
def continuous_algorithm_subgradient_penalty (data: ProblemData, seed_number, pruning=False, min_hour=None, max_hour=None):
    """
    Continuous algorithm for the MVT scheduling problem.
    Parameters:
    data (ProblemData): The problem data containing costs, time windows, and nurse requirements.
    ...
    # lam_pick = {(d, w): lam for d in range(day) for w in range(n)}
    # lam_drop = {(d, w): lam for d in range(day) for w in range(n)}
    # initialize lam to be 0
    lams = 0
    lam_pick = {(d, w): lams for d in range(day) for w in range(n)}
    lam_drop = {(d, w): lams for d in range(day) for w in range(n)}
