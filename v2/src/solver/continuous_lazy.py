...
def continuous_algorithm_lazy (data: ProblemData, work_limit, seed_number, multiple_tw=None, event_limit=None, pruning=1, min_hour=None, max_hour=None):
    """
    ...
    def depot_lazy_callback(model, where):
        if where == gp.GRB.Callback.MIPSOL:
            x_vals = model.cbGetSolution(model._x)
            alpha_vals = model.cbGetSolution(model._alpha)
            beta_vals = model.cbGetSolution(model._beta)
            ...
            for d in range(day):
                for w in range(n):
                    # Check AM depot rule
                    if sum(alpha_vals[j, d, w] for j in range(m)) > 1:
                        model.cbLazy(model._x[m, m+1, d, w] >= 1)

                    # Check PM depot rule
                    if sum(beta_vals[j, d, w] for j in range(m)) > 1:
                        model.cbLazy(model._x[m+2, m, d, w] >= 1)
    ...
    model.optimize(depot_lazy_callback)
    end_time = time.time()
