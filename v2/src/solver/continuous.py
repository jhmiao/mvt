...
def continuous_algorithm_fix_events (data: ProblemData, work_limit, seed_number, pruning=False, max_hour=None, chosen_schedules=None, feasible_arcs=None):
    """
    ...
    # Warm-start with chosen schedules: fix starts (and schedule) if provided
    if chosen_schedules:
        for d_key, schedule in chosen_schedules.items():
            d = int(d_key)
            for i_key, start_slot in schedule.items():
                i = int(i_key)
                # mark scheduled on that day
                s[i, d].Start = 1
                # provide start time hint
                t[i, d].Start = int(start_slot*30 + 570)

    # If feasible arcs are provided, zero-start arcs not in the allowed set (for i,j < m)
    if feasible_arcs:
        for d in range(day):
            # arcs that are allowed for this day
            allowed = feasible_arcs.get(str(d), set())

            # loop over ALL possible arcs (i,j)
            for i in range(m):
                for j in range(m):
                    if i == j:
                        continue

                    # if (i,j) is NOT in allowed arcs, hint it to 0
                    if (i, j) not in allowed:
                        for w in range(n):
                            x[i, j, d, w].VarHintVal = 0
                            x[i, j, d, w].VarHintPri = 100
...
def continuous_algorithm_discrete (data: ProblemData, work_limit, seed_number, multiple_tw=None, event_limit=None, pruning=False, min_hour=None, max_hour=None):
    """
    Continuous algorithm for the MVT scheduling problem.
    Parameters:
    data (ProblemData): The problem data containing costs, time windows, and nurse requirements.
    ...
    C_event0 = data.C_event
    C_home0 = data.C_home
    C_depot_e0 = data.C_depot_e
    C_depot_h0 = data.C_depot_h
    C_depot0 = np.concatenate([C_depot_e0, C_depot_h0])
    C_dur0 = data.C_dur
    time_window = data.time_window
    min_nurse = data.min_nurse
    nr = data.nr
    ...
    # take C_event, C_home, C_depot_e, C_depot_h, C_depot, C_dur, and time_window to the ceiling of value/30
    C_event = np.ceil(C_event0 / 30)
    # C_home = np.ceil(C_home0 / 30)
    # C_depot_e = np.ceil(C_depot_e0 / 30)
    # C_depot_h = np.ceil(C_depot_h0 / 30)
    # C_depot = np.ceil(C_depot0 / 30)
    C_dur = np.ceil(C_dur0 / 30)
    time_window = np.ceil(time_window / 30).astype(int)
    ...
    # Objective function
    event_cost = gp.quicksum(
        C_event0[i, j] * gp.quicksum(x[i, j, d, w] for d in range(day) for w in range(n)) for i in range(m) for j in range(m)
    )

    home_cost = gp.quicksum(
        C_home0[w,i] * gp.quicksum((x[i, m, d, w] + x[m, i, d, w]) for d in range(day)) for w in range (n) for i in range(m)
    )
    
    depot_event_cost = gp.quicksum(
        C_depot0[i] * gp.quicksum((x[m+1, i, d, w] + x[i, m+2, d, w] )for d in range(day) for w in range(n)) for i in range(m)
    )

    depot_home_cost = gp.quicksum(
        C_depot0[m+w] * gp.quicksum((x[m+2, m, d, w] + x[m, m+1, d, w]) for d in range(day)) for w in range(n)
    )
    ...
    M = 48  # large constant for time constraints
    # Time feasibility for consecutive events
    for i in range(m):
        for j in range(m):
            if i != j:
    ...
    if min_hour is not None:
        for w in range(n):
            model.addConstr(
                gp.quicksum(C_dur[j] * gp.quicksum(x[i, j, d, w] for i in range(m+3) for d in range(day)) for j in range(m)) >= min_hour[w] * 2,
                name=f"min_working_hours_w{w}"
            )
    ...
    if max_hour is not None:
        for w in range(n):
            model.addConstr(
                gp.quicksum(C_dur[j] * gp.quicksum(x[i, j, d, w] for i in range(m+3) for d in range(day)) for j in range(m)) <= max_hour[w] * 2,
                name=f"max_working_hours_w{w}"
            )
    ...
    model.setParam("PoolSearchMode", 2)      # 2 = do a full search for multiple sols
    model.setParam("PoolSolutions", 100)     # max number of solutions you want 
