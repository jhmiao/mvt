# src/solver/continuous_no_depot.py
...
def continuous_algorithm_no_depot_ratio (data: ProblemData, work_limit, seed_number, multiple_tw=None, event_limit=None, pruning=False, min_hour=None, max_hour=None):
    """
    ...
    # modify C_home by C_depot_h ratio
    # normalize C_depot_h to have minimum of 1
    C_depot_h_norm = C_depot_h / np.min(C_depot_h)

    # again, normalize these ratios to have maximum of 1.2
    C_depot_h_norm = np.minimum(C_depot_h_norm, 1.2)

    # modify C_home by applying the same ratio
    for w in range(n):
        C_home[w, :] = C_home[w, :] * C_depot_h_norm[w]
    ...
    # i, j == m for home
    x = model.addVars(m+1, m+1, day, n, vtype=GRB.BINARY, name="x")
    ...
    objective = event_cost + home_cost
    ...
    for i in range(m):
        for d in range(day):
            for w in range(n):
                model.addConstr(sum(x[i,j,d,w] for j in range(m+1)) <= s[i,d], name=f"event_flow_i{i}_d{d}_w{w}")
    ...
    if min_hour is not None:
        for w in range(n):
            model.addConstr(
                gp.quicksum(C_dur[j] * gp.quicksum(x[i, j, d, w] for i in range(m+1) for d in range(day)) for j in range(m)) >= min_hour[w] * 60,
    ...
    if max_hour is not None:
        for w in range(n):
            model.addConstr(
                gp.quicksum(C_dur[j] * gp.quicksum(x[i, j, d, w] for i in range(m+1) for d in range(day)) for j in range(m)) <= max_hour[w] * 60,
    ...
    for j in range(m):
        model.addConstr(
            gp.quicksum(x[i, j, d, w] for i in range(m+1) for d in range(day) for w in range(nr)) >= min_nurse[j][0],
            name=f"min_RN_j{j}"
        )
        model.addConstr(
            gp.quicksum(x[i, j, d, w] for i in range(m+1) for d in range(day) for w in range(nr, nr+nl)) >= min_nurse[j][1],
            name=f"min_LVN_j{j}"
        )
    ...
    for j in range(m):
        for d in range(day):
            for w in range(n):
                model.addConstr(
                    gp.quicksum(x[i, j, d, w] for i in range(m+1)) == gp.quicksum(x[j, i, d, w] for i in range(m+1)),
                    name=f"event_network_flow_j{j}_d{d}_w{w}"
                )

    # outflow from home is at most 1
    model.addConstrs(
        (gp.quicksum(x[m, i, d, w] for i in range(m)) <= 1 for d in range(day) for w in range(n)),
        name="home_outflow"
    )

    # inflow to home is at most 1
    model.addConstrs(
        (gp.quicksum(x[i, m, d, w] for i in range(m)) <= 1 for d in range(day) for w in range(n)),
        name="home_inflow"
    )
